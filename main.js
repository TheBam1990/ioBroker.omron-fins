...
class OmronFins extends utils.Adapter {
  ...
  async onReady() {
    ...
    this.devices = mergeDeviceTables(
      Array.isArray(this.config.devices) ? this.config.devices : [],
      imported,
    );
    if (imported.length) {
      this.log.info(
        `Imported ${imported.length} symbols from the CX-Programmer table`,
      );
    }
    if (!this.config.plc_ip || !this.devices.length) {
      await this.setError(
        !this.config.plc_ip
          ? "PLC IP address is missing"
          : "No PLC variables are configured",
      );
      return;
    }
    ...
    try {
      ...
      this.client.on(
        "error",
        (error) =>
          void this.setError(`FINS client error: ${errorMessage(error)}`),
      );
      this.schedulePoll(0);
    } catch (error) {
      await this.setError(
        `Cannot initialize FINS client: ${errorMessage(error)}`,
      );
    }
  }
  ...
  async poll() {
    ...
    this.polling = true;
    try {
      const message = await this.readMultiple(
        this.devices.map((device) => device.address),
      );
      const values =
        message.values || (message.response && message.response.values) || [];
            ...
            error ||
              new Error(
                message && message.response
                  ? message.response.endCodeDescription
                  : "PLC timeout",
              ),
          );
        } else {
  ...
  async onStateChange(id, state) {
    ...
    const relative = id.startsWith(`${this.namespace}.`)
    ...
    try {
      const value =
        device.type === "BOOL" ? (state.val ? 1 : 0) : Number(state.val);
      ...
      await this.setStateAsync(
        relative,
        device.type === "BOOL" ? Boolean(value) : value,
        true,
      );
      await this.setStateAsync("info.connection", true, true);
    } catch (error) {
      await this.setError(
        `FINS write failed for ${device.address}: ${errorMessage(error)}`,
      );
    }
  }
  ...
  armConnectionTimeout() {
    ...
    const delay =
      Math.max(Number(this.config.plc_poll) || 5000, 250) +
      Math.max(Number(this.config.timeout) || 5000, 1000);
