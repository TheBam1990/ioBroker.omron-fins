 ...
 * @returns {Array<Record<string, any>>} Imported devices
 */
function parseSymbolTable(text) {
  ...
  const find = (terms) =>
    header.findIndex((value) => terms.some((term) => value.includes(term)));
  ...
  for (const row of rows.slice(start)) {
    const detectedAddress = row.findIndex((value) =>
      addressPattern.test(value.replace(/\s/g, "")),
    );
    ...
    const namePosition =
      nameIndex >= 0 ? nameIndex : Math.max(0, addressPosition - 1);
    const name =
      row[namePosition] || `PLC_${normalizeAddress(row[addressPosition])}`;
    result.push({
      name,
 ...
 *
 * @param {Array<Record<string, any>>} manual Manual devices
 * @param {Array<Record<string, any>>} imported Imported devices
 * @returns {Array<Record<string, any>>} Merged devices
 */
